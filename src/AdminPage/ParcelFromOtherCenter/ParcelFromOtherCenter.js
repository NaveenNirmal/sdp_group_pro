...
import './ParcelFromOtherCenter.css';



function ParcelFromOtherCenter() {
    ...
    const onClick = async (e) => {
        ...
        await fireDb.collection('tracking').doc(documentId).update({ArrivedAtWarehouse:true}).then((res) => {
            console.log(res);
            toast.success("Parcel Received to Warehouse")
        }).catch((err) => {
            console.log(err);
            toast.error(`${err}`)
        })

        await fireDb.collection('order_requests').doc(documentId).update({request_status:'Arrived At Warehouse'}).then((res) => {
            console.log(res);
        }).catch((err) => {
            console.log(err);
        })
    ...
    const data = {
        ...
        rows:collectedData.map((item) => {
            return{
                ...
                action:<button className="btn btn-info btn-sm" value={item.id} onClick={onClick}>Confirm Parcel Recieved</button>,
            ...
            <div className="row">
                <div className="col-md-12">
                     <div className="collectParcel__Header">
                        <h4 className="collectParcel__Heading">Recived Parcel From Other Center</h4>
                    </div>
                </div>
            </div>
...
export default ParcelFromOtherCenter